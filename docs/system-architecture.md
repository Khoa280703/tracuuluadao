...
The system now has two storage layers:
- **Ephemeral cache** in PostgreSQL for scraper results and agent outputs with TTL
- **Persistent knowledge base** in PostgreSQL for subjects, investigations, evidence, links, and community reports
...
- Current link type created by ingest is `mentioned_together`.
